                ...
                try:
                    ...
                    if content["type"] == "image" and len((content.get("ocr") or "").strip()) < 5:
                        if not ocr_available():
                            st.error(
                                "На сервере не установлен OCR (tesseract), а текущий API-прокси "
                                "не передаёт изображения в модель. Добавьте файл packages.txt "
                                "(tesseract-ocr, tesseract-ocr-rus) и перезапустите приложение, "
                                "либо загружайте работы в формате PDF/DOCX с текстом.")
                        else:
                            st.error(
