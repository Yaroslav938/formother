"""
app.py — Streamlit-приложение проверки работ учеников через Google Gemini.

Доработки:
- Проверка через Gemini (Vision): фото/сканы читаются моделью напрямую,
  включая рукописный текст и кириллицу. Локальный OCR больше не нужен.
- Выбор модели Gemini (Flash / Pro) прямо в интерфейсе.
...
with st.sidebar:
    ...
    api_key = st.text_input("🔑 Gemini API Key", type="password", placeholder="AIza...",
                            help="Ключ из Google AI Studio: https://aistudio.google.com/apikey")
    model = st.selectbox("🤖 Модель Gemini", get_models(),
                         index=get_models().index(DEFAULT_MODEL),
                         help="Flash — быстрее и доступна на бесплатном тарифе; "
                              "Pro — точнее, но требует большей квоты.")
    subject = st.selectbox("📚 Предмет", SUBJECTS)
    st.divider()
    st.markdown("**Поддерживаемые форматы:**\n\nPDF, DOCX, JPG, PNG")
    st.caption("Фото и сканы читаются моделью Gemini напрямую (включая рукописный текст и кириллицу).")
    st.divider()
    debug_mode = st.checkbox("🐞 Режим диагностики",
                            help="Показывает технические детали: что уходит в модель и что она вернула.")
    cbtn1, cbtn2 = st.columns(2)
    if cbtn1.button("🔑 Проверить ключ", use_container_width=True):
        if not api_key:
            st.warning("Сначала введите ключ.")
        else:
            with st.spinner("Проверяю ключ..."):
                chk = check_api_key(api_key, model)
            if chk.get("ok"):
                st.success("✅ Ключ работает.")
            else:
                st.error(f"❌ {chk.get('error', 'Ключ не работает.')}")
    if cbtn2.button("🔍 Тест фото", use_container_width=True,
                    help="Проверяет, читает ли модель текст с изображения."):
        if not api_key:
            st.warning("Сначала введите ключ.")
        else:
            with st.spinner("Проверяю чтение фото..."):
                diag = diagnose_vision(api_key, model)
            if diag.get("error"):
                st.error(f"Ошибка запроса: {diag['error']}")
            elif diag.get("vision_ok"):
                st.success("✅ Модель читает текст с фото.")
            else:
                st.error("❌ Модель не распознала текст на тестовой картинке.")
            st.json(diag)
    st.divider()
    df_side = load_grades()
...
if not api_key:
    ...
    <div class="hero">
      ...
      <p>Загрузите фото или документ — Gemini проверит работу, выставит оценку и даст разбор по критериям.</p>
    </div>
    """, unsafe_allow_html=True)
    st.warning("Введите Gemini API ключ в боковой панели, чтобы начать. "
               "Ключ можно получить бесплатно на https://aistudio.google.com/apikey")
    st.stop()
...
with tab1:
    ...
    if uploaded:
        ...
        if run:
            with st.spinner("Gemini анализирует работу..."):
                try:
                    ...
                    # Понятная ошибка от модели (квота/перегрузка/ключ) — НЕ «всё правильно»
                    st.error(f"⚠\ufe0f {e}")
                    st.stop()
