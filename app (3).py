...
def build_content_blocks(uploaded_files, api_format: str = "anthropic") -> tuple[list, list[str]]:
    ...
    for f in uploaded_files:
        ...
        if ext in ("jpg", "jpeg", "png", "webp", "gif", "bmp", "tiff"):
            ...
            pdf_limit = 18_000_000 if api_format == "gemini" else 30_000_000
            if len(data) > pdf_limit:
                if api_format == "gemini" and fitz is not None:
                    # слишком большой для inline — рендерим страницы в картинки
                    try:
                        blocks.extend(pdf_to_image_blocks(data, name))
                    except Exception as e:
                        warnings.append(f"Не удалось обработать PDF {name}: {e}")
                else:
                    warnings.append(f"PDF {name} слишком большой ({len(data) // 1_000_000} МБ) — пропущен.")
                continue
            if api_format == "openai":
                ...
                # Anthropic и Gemini принимают PDF нативно
...
def to_gemini_parts(blocks: list) -> list:
    """Конвертирует внутренние блоки в формат Gemini (parts с inline_data)."""
    parts = []
    for b in blocks:
        if b["type"] == "text":
            ...
            src = b["source"]  # Gemini принимает PDF нативно как inline_data
            parts.append({"inline_data": {"mime_type": src["media_type"], "data": src["data"]}})
    return parts
...
def call_claude(api_key: str, base_url: str, auth_mode: str, model: str,
    ...
    """Прямой HTTP-вызов API. Форматы: Google Gemini (generateContent),
    ...
    if api_format == "gemini":
        ...
        payload = {
            ...
            "contents": [{"role": "user", "parts": to_gemini_parts(content_blocks)}],
            ...
            if resp.status_code == 200:
                ...
                if api_format == "gemini":
                    ...
                    if not cands:
                        ...
                        raise RuntimeError(f"Gemini не вернул ответ (возможно, сработал фильтр): {fb}")
...
def make_test_image_b64() -> str:
    """Картинка 200x200 с красным квадратом для теста зрения."""
...
def run_vision_test(api_key, base_url, auth_mode, model, api_format) -> tuple[bool, str]:
    """Возвращает (видит_ли_картинку, ответ_или_ошибка)."""
    test_blocks = [
        {"type": "text", "text": "Ответь одним-двумя словами: какого цвета квадрат на картинке?"},
        {"type": "image", "source": {"type": "base64", "media_type": "image/jpeg",
                                     "data": make_test_image_b64()}},
    ]
    try:
        reply = call_claude(api_key, base_url, auth_mode, model,
                            "Ты видишь изображения. Отвечай кратко.",
                            test_blocks, api_format=api_format,
                            max_tokens=50, timeout=60)
        ok = any(w in reply.lower() for w in ("красн", "red"))
        return ok, reply.strip()[:200]
    except Exception as e:
        return False, f"ошибка: {str(e)[:200]}"


def fetch_model_list(api_key, base_url, api_format="anthropic") -> list[str]:
    """Получает список доступных моделей провайдера."""
    url = base_url.rstrip("/")
    if api_format == "gemini":
        if not url.endswith("/models"):
            url += "/models" if url.endswith("/v1beta") else "/v1beta/models"
        resp = httpx.get(url, headers={"x-goog-api-key": api_key}, timeout=30)
        if resp.status_code != 200:
            raise RuntimeError(f"HTTP {resp.status_code}: {resp.text[:200]}")
        models = resp.json().get("models", [])
        return [m.get("name", "").replace("models/", "") for m in models if m.get("name")]

    if not url.endswith("/models"):
        url += "/models" if url.endswith("/v1") else "/v1/models"
    errors = []
    ...
    raise RuntimeError("; ".join(errors) or "не удалось получить список")
...
with st.sidebar:
    ...
    provider = st.selectbox(
        "Провайдер",
        ["Google Gemini", "Anthropic / реселлер (/v1/messages)", "OpenAI-совместимый (/v1/chat/completions)"],
        help="Gemini: ключ бесплатно на https://aistudio.google.com/apikey",
    )
    if provider.startswith("Google"):
        api_format = "gemini"
        default_url = "https://generativelanguage.googleapis.com"
        provider_models = ["gemini-2.5-flash", "gemini-2.5-pro", "gemini-2.0-flash"]
    ...
    api_key = st.text_input("API-ключ", type="password", key=f"key_{api_format}",
                            help="Для Gemini — ключ из Google AI Studio (начинается с AIza…)")
    base_url = st.text_input("Base URL", value=default_url, key=f"url_{api_format}",
                             help="Для Gemini обычно менять не нужно. Для реселлера — его эндпоинт.")
    if api_format == "gemini":
    ...
    model_choice = st.selectbox("Модель", provider_models + ["Другая…"], key=f"model_{api_format}")
    model = st.text_input("Название модели", value="", key=f"custom_{api_format}") \
        if model_choice == "Другая…" else model_choice
    ...
    if st.button("👁 Тест подключения и зрения", use_container_width=True,
        ...
        if not api_key or not model.strip():
            ...
            with st.spinner("Проверяю…"):
                ok, reply = run_vision_test(api_key, base_url, auth_mode, model.strip(), api_format)
            if ok:
                st.success(f"✅ Зрение работает. Ответ модели: {reply}")
            else:
                st.error(f"⚠️ Картинка до модели не доходит. Ответ: «{reply}». "
                         "Нажмите «Автоподбор настроек» ниже.")

    if api_format != "gemini" and st.button(
            "🧪 Автоподбор настроек", use_container_width=True,
            help="Прогоняет тест зрения по всем комбинациям формата API и авторизации."):
        if not api_key or not model.strip():
            st.error("Сначала укажите ключ и модель.")
        else:
            combos = [(f, a) for f in ("anthropic", "openai")
                      for a in ("x-api-key", "Authorization: Bearer")]
            rows, found = [], None
            prog = st.progress(0.0, text="Тестирую комбинации…")
            for i, (fmt, auth) in enumerate(combos):
                ok, reply = run_vision_test(api_key, base_url, auth, model.strip(), fmt)
                rows.append({
                    "Формат API": "Anthropic" if fmt == "anthropic" else "OpenAI-совм.",
                    "Авторизация": auth,
                    "Зрение": "✅ да" if ok else "❌ нет",
                    "Ответ / ошибка": reply,
                })
                if ok and found is None:
                    found = (fmt, auth)
                prog.progress((i + 1) / len(combos), text=f"Проверено {i + 1} из {len(combos)}")
            prog.empty()
            st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
            if found:
                st.success(f"✅ Рабочая комбинация: формат **{'Anthropic' if found[0] == 'anthropic' else 'OpenAI-совместимый'}** + "
                           f"авторизация **{found[1]}**. Выберите её в настройках выше.")
            else:
                st.error("Ни одна комбинация не работает с картинками — под этим названием модели "
                         "у реселлера, скорее всего, модель без зрения. Нажмите «Показать модели» "
                         "и попробуйте другое название (модель с поддержкой vision).")

    if st.button("📃 Показать доступные модели", use_container_width=True,
                 help="Запрашивает у провайдера список доступных моделей."):
        if not api_key:
            st.error("Сначала укажите ключ.")
        else:
            try:
                with st.spinner("Запрашиваю список…"):
                    ids = fetch_model_list(api_key, base_url, api_format)
                st.success(f"Доступно моделей: {len(ids)}")
                st.dataframe(pd.DataFrame({"Модель": sorted(ids)}),
                             use_container_width=True, hide_index=True, height=300)
                st.caption("Скопируйте нужное название и вставьте через пункт «Другая…».")
            except Exception as e:
                st.error(f"Не удалось получить список моделей: {e}. "
                         "Посмотрите названия в документации или личном кабинете реселлера.")
