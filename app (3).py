...
def build_content_blocks(uploaded_files) -> tuple[list, list[str]]:
...
def call_claude(api_key: str, base_url: str, auth_mode: str, model: str,
    ...
    """Прямой HTTP-вызов /v1/messages — совместим с реселлерскими эндпоинтами."""
    url = base_url.rstrip("/")
...
if run:
    ...
    blocks, warnings = build_content_blocks(uploaded_files)
    for w in warnings:
        st.warning(w)
    if not blocks:
    ...
    with st.spinner("Проверяю работу… Это может занять до пары минут."):
        try:
            raw = call_claude(api_key, base_url, auth_mode, model.strip(), system_prompt, blocks)
