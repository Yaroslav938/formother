...
def fetch_model_list(api_key, base_url) -> list[str]:
    """Пробует получить список моделей реселлера через GET /v1/models (обоими способами авторизации)."""
    url = base_url.rstrip("/")
...
with st.sidebar:
    ...
    if st.button("🧪 Автоподбор настроек", use_container_width=True,
    ...
    if st.button("📃 Показать модели реселлера", use_container_width=True,
                 help="Запрашивает список доступных моделей через /v1/models."):
        if not api_key:
            st.error("Сначала укажите ключ.")
        else:
            try:
                with st.spinner("Запрашиваю список…"):
                    ids = fetch_model_list(api_key, base_url)
